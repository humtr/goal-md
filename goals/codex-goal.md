...
| Dimension | Current | Target | Reason |
| --- | ---: | ---: | --- |
| Goal record hygiene | 98 | 99.6+ | Externalize the goal ledger and keep codex product-only. |
| Thin wrapper philosophy | 89 | 97.8+ | Leave shell as Termux glue, not policy or parsing logic. |
| Refactor quality | 92 | 98.4+ | Prevent Python CLI monolith growth and tighten ownership. |
| Product validation confidence | 96 | 99.2+ | Add mismatch, tuple, network-disabled, and doctor-field proof. |
| Merge candidate maturity | 93 | 98.6+ | Remove non-product artifacts and leave a clean, reviewable stack. |
...
- Latest pushed commit at goal creation: `dbbabbd Complete Python boundary expansion proof`
- Installed wrapper proof from the prior goal:
  - wrapper: `260702-11 (dbbabbd86348)`
...
- `lib_lines` <= 90
- `install_runtime_lines` <= 500
- `domain_shell_lines` <= 2100
- `lib_shell_functions` <= 130
- `runtime_shell_lines` <= 650
- `state_shell_lines` <= 330
- `notify_shell_functions` <= 16
- `profile_shell_functions` <= 12
...
- move remaining runtime action selection and status interpretation into
  Python modules.
- move structured state formatting and field extraction into Python.
...
- shell line/function counts move toward Phase 2 budgets.
...
- `bin/install-runtime.sh` <= 500 lines, or any remaining excess is justified
...
- `tools/codex_termux/cli.py` is materially smaller.
...
- [ ] Phase 0 external goal ledger complete.
- [ ] Phase 1 product purity cleanup complete.
- [ ] Phase 2 stricter shell budgets implemented.
...
- [ ] `humtr/codex` has no in-repo `GOAL.md`.
...
- [ ] no branch pushed unless explicitly authorized.

## Not Proven Yet

- Whether the aggressive shell budgets can all be reached without making the
  wrapper less clear.
- Whether `cli.py` can be split cleanly without excessive import complexity.
